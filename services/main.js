...
const checkMeetingVariables = () => {
  ...
  if (getFromStore("MEETING_VARIABLES")) {
    console.log(
      "meeting is establishing again",
      JSON.parse(getFromStore("MEETING_VARIABLES"))
    );
...
const connectUserToLive = (meetInfo) => {
  ...
  if (isUrlChanged) console.log("emit url changes");
...
const showNotification = (isAgentAvailable = true) => {
  ...
  if (!isAgentAvailable)
    notifications = [
      ...
      {
        icon: "fa-envelope",
        msg: "Will get in touch with you via email soon!",
      },
    ];
  let count = 0;
...
const validate = (id) => {
  ...
  for (const element of elements) {
    ...
    if (value && value !== "Select a Inquiry Type")
      field.classList.remove("mtx-form-control-error");
...
const submit = async () => {
  socket = io.connect(socketUrl, { query: { appId } });
  // AIYASH - socket id will be rturn in response here so can get it and assign it to visitorSocketId for future usage
